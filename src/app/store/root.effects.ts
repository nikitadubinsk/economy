...
export class RootEffects {
    ...
    showErrorMessage$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                map(({message, options}) => this.alertService.showErrorMessage(message, options))
            ),
        {dispatch: false}
    );
        ...
        this.actions$.pipe(
            ...
            switchMap(({login, password}) =>
                this.apiAuthService.authorize({login, password}).pipe(
                    switchMap(({token}) => [setLS({token}), loadUserInfo(), turnOffLoadingButton()]),
                    catchError(() => [showErrorMessage({message: 'Произошла ошибка'}), turnOffLoadingButton()])
                )
            )
        )
    );
        ...
        this.actions$.pipe(
            ...
            switchMap(() =>
                this.apiAuthService.userInfo().pipe(
                    switchMap(({name, role, operatorRoles, typeInformationPerception}) => [
    ...
    loadUserPhoto$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({photo, typeInformationPerception, description, telegramUserId}) =>
                this.apiAuthService.loadUserPhoto(photo).pipe(
                    switchMap(({name}) => [
                        updateUserInfo({typeInformationPerception, description, telegramUserId, photo: name}),
                    ]),
                    catchError(() => [showErrorMessage({message: 'Произошла небольшая ошибка, попробуйте еще раз'})])
                )
            )
        )
    );
    ...
    updateUserInfo$ = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(({photo, typeInformationPerception, description, telegramUserId}) =>
                this.apiAuthService.updateUserInfo(typeInformationPerception, description, telegramUserId, photo).pipe(
        ...
        this.actions$.pipe(
            ...
            switchMap(({login}) =>
                this.apiAuthService.resetPassword(login).pipe(
                    switchMap(() => [
                        navigateTo({payload: {path: ['/auth']}}),
                        showSuccessMessage({message: 'Вы успешно сбросили пароль. Новый пароль уже на почте'}),
                        turnOffLoadingButton(),
                    ]),
                    catchError(() => [
                        showErrorMessage({message: 'Произошла ошибка'}),
