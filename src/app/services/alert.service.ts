...
export class AlertService {
    ...
    showSuccessMessage(msg = NOTIFICATION_ALERTS.SUCCESS, options: Partial<TuiAlertOptions<void>> = {}): void {
        this.showMessage(msg, {...options, status: TuiNotification.Success});
    }

    showWarningMessage(msg: string, options: Partial<TuiAlertOptions<void>> = {}): void {
        this.showMessage(msg, {...options, status: TuiNotification.Warning});
    }

    showErrorMessage(msg = NOTIFICATION_ALERTS.ERROR, options: Partial<TuiAlertOptions<void>> = {}): void {
        this.showMessage(msg, {...options, status: TuiNotification.Error});
    }

    showInfoMessage(msg: string, options: Partial<TuiAlertOptions<void>> = {}): void {
