...
export abstract class ApiService {
    ...
    protected get<T, S = IParams, E = IHttpMetaData>(endpoint: string, params?: S): Observable<IApiResponse<T, E>> {
        return this.request(EHttpMethod.GET, endpoint, params);
    }

    protected post<T, S = IParams, P = IParams>(endpoint: string, body?: S, params?: P): Observable<IApiResponse<T>> {
        return this.request(EHttpMethod.POST, endpoint, params, body);
    }

    protected put<T, S = IParams, P = IParams>(endpoint: string, body?: S, params?: P): Observable<IApiResponse<T>> {
        return this.request(EHttpMethod.PUT, endpoint, params, body);
    }

    protected delete<T, S = IParams>(endpoint: string, params?: S, body?: S): Observable<IApiResponse<T>> {
        return this.request(EHttpMethod.DELETE, endpoint, params, body);
    }

    protected sendFile<T, P = IParams, E = IHttpMetaData>(endpoint: string, file: File, params?: P) {
    ...
    private parseEndpoint(endpoint: string, params: IHttpParams): {url: string; queryParams: IHttpParams} {
    ...
    private resolveUrl(url: string): string {
        return `${this._baseUrl}${encodeURI(url)}`.replace(/(^|[^:/]|\/)(\/{2,})/g, '$1/');
    }
    ...
    private parseData<T, E = IHttpMetaData>(response: IApiResponse<T, E> | null): IApiResponse<T, E> {
        return response && response.data ? response : {data: (response as unknown) as T, meta: undefined, errors: []};
    }
}
